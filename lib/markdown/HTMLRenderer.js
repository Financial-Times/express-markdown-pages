...
class HTMLRenderer extends Renderer {
	...
	link(href, title, text) {
		// Transform local links between Markdown files into URL slugs.
		...
		if (isInternalLink(href)) {
			href = createPageSlug(decodeURI(href));
		}
	...
	image(href, title, text) {
		// Transform references to local images into URL slugs.
		if (isInternalImage(href)) {
			href = createFileSlug(href);
		}
