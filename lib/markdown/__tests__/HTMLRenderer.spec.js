...
describe('lib/markdown/HTMLRenderer', () => {
	...
	describe('.link()', () => {
		it('transforms local link URLs between Markdown files into friendly URLs', () => {
			const resultA = renderer.link('../A%20Page.md');
			expect(resultA).toMatch('href="../a-page"');

			const resultB = renderer.link('./path/to/A%20Page.md');
			expect(resultB).toMatch('href="./path/to/a-page"');
		});

		it('transforms local link URLs to images into friendly URLs', () => {
			const resultA = renderer.link('../An%20Image.png');
			expect(resultA).toMatch('href="../an-image.png"');

			const resultB = renderer.link('./path/to/A%20cool%20photo.jpg');
			expect(resultB).toMatch('href="./path/to/a-cool-photo.jpg"');
		});

		it('does not transform external link URLs', () => {
			const resultA = renderer.link('//www.ft.com/A%20Page.md');
	...
	describe('.image()', () => {
		it('transforms local image URLs into friendly URLs', () => {
		...
		it('does not transform external image URLs', () => {
