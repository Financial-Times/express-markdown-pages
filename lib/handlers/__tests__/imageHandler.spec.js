...
describe('lib/handlers/imageHandler', () => {
	...
	describe('when an image is found', () => {
		...
		beforeAll(() => {
			const image = images.by('slug', '/a-nice-photo.jpg');
			// This handler expects the image as the first argument but
			// this test suite will send mock requests directly to it.
			routeHandler = subject.imageHandler.bind(null, image);
		});
	...
	describe('when an image is not found', () => {
		...
		beforeAll(() => {
			const image = images.by('slug', '/a-non-existant-photo.jpg');
			// This handler expects the image as the first argument but
			// this test suite will send mock requests directly to it.
			routeHandler = subject.imageHandler.bind(null, image);
		});
