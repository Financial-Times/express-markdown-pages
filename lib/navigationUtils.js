 ...
 * @returns {Page[]}
 */
function filterOutDraftPages(pages, excludeDrafts) {
	return pages.filter(page => !(excludeDrafts && page.draft === true));
}
...
function getNavigation(page, collection, excludeDrafts = false) {
	const ancestors = filterOutDraftPages(
		getAncestors(page, collection),
		excludeDrafts,
	);

	const siblings = filterOutDraftPages(
		getSiblings(page, collection),
		excludeDrafts,
	);

	const children = filterOutDraftPages(
		getChildren(page, collection),
		excludeDrafts,
	);
