...
/**
 * Create redirect slug
 * @description Creates an absolute path for the target page
 * @param {String} currentPagePath
 * @param {String} redirectPagePath
 * @returns {String}
 */
function createRedirectSlug(currentPagePath, redirectPagePath) {
	// We need to remove the filename from the current page path because
	// Node's path methods don't know the difference between files and folders.
	const basePath = path.dirname(currentPagePath);
	return createPageSlug(path.join(basePath, redirectPagePath));
}
