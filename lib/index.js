...
const imageFileHandler = require('./handlers/imageFileHandler');
...
class MarkdownPages {
	...
	async middleware(request, response, next) {
		...
		try {
			...
			if (/\.(png|gif|jpg|svg|webp)$/.test(request.path)) {
				return imageFileHandler(db, request, response, next);
			}
			...
			return pageHandler(db, request, response, next);
		} catch (error) {
			next(error);
		}
	}
	...
	/**
	 * Init
	 * @returns {Promise<import('lokijs').Loki>}
	 */
	init() {
