const fs = require('fs');
const glob = require('glob');
const throat = require('throat').default;
const zip = require('just-zip-it');

/**
 * Find files
 * @param {String} globPattern
 * @returns {Promise<String[]>}
 */
function findFiles(globPattern) {
	return new Promise((resolve, reject) => {
		glob(globPattern, {}, (error, files) => {
			if (error) {
				reject(error);
			} else {
				resolve(files);
			}
		});
	});
}

/**
 * Load files
 * @param {String[]} filePaths
 * @returns {Promise<String[]>}
 */
function loadFiles(filePaths = []) {
	const throttle = throat(2);

	const queue = filePaths.map(filePath => {
		return throttle(() => fs.promises.readFile(filePath, 'utf-8'));
	});

	return Promise.all(queue);
}

/**
 * Get files
 * @param {String} globPattern
 * @returns {Promise<Array<String[]>>}
 */
 async function getFiles(globPattern) {
	// NOTE: we must mock findFiles() and loadFiles() to test this method
	// but in the test file we can only access the references exported so
	// we'll use the same references here to make mocking them easy.
	// <https://github.com/facebook/jest/issues/936>
	const files = await module.exports.findFiles(globPattern);
	const contents = await module.exports.loadFiles(files);

	return zip(files, contents);
}

module.exports = { findFiles, loadFiles, getFiles };
