...
const { getIndexFromPath, createSlug } = require('./linkUtils');
// const {
...
function createPageFromFile(options, filePath, fileContent) {
	...
	const slug = urlJoin(options.pathPrefix, createSlug(relativePath));
	...
	return {
		...
		id: nanoid(),
		index: getIndexFromPath(relativePath) || Infinity,
		slug,
		body,
