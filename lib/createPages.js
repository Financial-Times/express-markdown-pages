...
const { getIndexFromPath, createPageSlug, safeUrlJoin } = require('./linkUtils');
// const {
...
function createPageFromFile(options, filePath, fileContent) {
	...
	const slug = safeUrlJoin(options.pathPrefix, createPageSlug(relativePath));
...
function addRelationshipData(pages) {
	pages.forEach(page => {
		...
		// NOTE: we use safe URL join to avoid creating any double slashes (//)
		const slugPattern = safeUrlJoin('^', page.slug, '[^/]+$');
