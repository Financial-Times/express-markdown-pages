...
describe('lib/createPages', () => {
	// This method only calls glob() so there's nothing to test
	...
	describe('.addRelationshipData()', () => {
		...
		it('finds children for each page based on their slug', () => {
			...
			expect(page2.childIDs).toEqual([page3.id]);
			expect(page3.childIDs).toEqual([page4.id, page5.id]);
			expect(page4.childIDs).toEqual([]);
			expect(page5.childIDs).toEqual([]);
		});
		...
		it('appends a parent ID to each child', () => {
			...
			expect(page4.parentID).toEqual(page3.id);
			expect(page5.parentID).toEqual(page3.id);
		});
	});
	...
	// This method calls the functions tested individually above
